	...
	if item.itemid == button then
		...
		for a = 0, getContainerSize(stone_slot.uid) - 1 do
			...
			if it.uid > 0 then
				...
				totalStones = totalStones + (it.type ~= 0 and it.type or 1)
			end
		end

		local baseCost     = stones_required[stone.itemid]
		local inc          = stone_increase[stone.itemid]
		local currentBoost = boost
		local removeStones = 0
		local boosts       = 0
		local maxBoost     = 50

		while currentBoost < maxBoost do
			local cost = baseCost + math.floor(currentBoost / inc)
			if totalStones < cost then break end
			totalStones  = totalStones - cost
			removeStones = removeStones + cost
