...
function onUse(cid, item, frompos, item2, topos)
    ...
    getPlayerStorageValue(cid, 63215) >= 1 or getPlayerStorageValue(cid, 75846) >= 1 then
    return true
    end
    ...
    if item.itemid == usando then
        ...
        if ball and ball.uid > 0 then
            doItemSetAttribute(ball.uid, "mega_active", 0)
        end
    ...
    elseif item.itemid == pokeballs[btype].on then
        ...
        if shinysClan[pokemon] and (getPlayerGroupId(cid) < 4 or getPlayerGroupId(cid) > 6) then -- alterado v1.9 \/
            if getPlayerClanNum(cid) ~= shinysClan[pokemon][1] then
                doPlayerSendCancel(cid, "You need be a member of the clan " .. shinysClan[pokemon][2] ..
                    " to use this pokemon!")
                return true
        ...
        if getCreatureName(pk) == "Ditto" or getCreatureName(pk) == "Shiny Ditto" then
            local left = getItemAttribute(item.uid, "transLeft")
            local name = getItemAttribute(item.uid, "transName")
            local outfit = getItemAttribute(item.uid, "transOutfit")
            ...
            if name then
                ...
                elseif left and left > 0 then
                    ...
                    setPlayerStorageValue(pk, 1010, getCreatureName(pk) == "Ditto" and "Ditto" or "Shiny Ditto")
                end
            else
                setPlayerStorageValue(pk, 1010, getCreatureName(pk) == "Ditto" and "Ditto" or "Shiny Ditto")
            end
            sendDittoMemorySync(getCreatureMaster(pk))
        end
        ...
        doItemSetAttribute(item.uid, "10002", pokemon .. "_off")
