...
[12854] = { segs = 5, pokes = {{"Seel", 2}, {"Chinchou", 2}, {"Squirtle", 3}} },

[12858] = { segs = 5, pokes = {{"Seaking", 4}, {"Seadra", 3}, {"Poliwhirl", 4}} },

[12857] = { segs = 5, pokes = {{"Starmie", 2}, {"Kingler", 2}, {"Corsola", 2}, {"Qwilfish", 2}, {"Wartortle", 2}} },  -- pega no client da pxg
...
[12853] = { segs = 5, pokes = {{"Gyarados", 3}, {"Mantine", 3}, {"Tentacruel", 3}, {"Kingdra", 3}, {"Blastoise", 3}, {"Giant Magikarp", 1}} },
}
...
local function doFish(cid, pos, ppos, interval)
      ...
      if getPlayerSkillLevel(cid, 6) < limite then 
		...
		elseif pesca >= 21 and pesca <= 26 then
			doPlayerAddSkillTry(cid, 6, bonus * 200)	
		elseif pesca >= 27 and pesca <= 30 then
			doPlayerAddSkillTry(cid, 6, bonus * 350)
		elseif pesca >= 31 and pesca <= 33 then
			doPlayerAddSkillTry(cid, 6, bonus * 1500)	
		elseif pesca >= 34 and pesca <= 37 then
			doPlayerAddSkillTry(cid, 6, bonus * 2000)	
		elseif pesca >= 38 and pesca <= 41 then
			doPlayerAddSkillTry(cid, 6, bonus * 9000)	
		elseif pesca >= 42 and pesca <= 46 then
			doPlayerAddSkillTry(cid, 6, bonus * 25000)	
		elseif pesca >= 47 and pesca <= 48 then
			doPlayerAddSkillTry(cid, 6, bonus * 70000)
		elseif pesca >= 49 and pesca <= 50 then
			doPlayerAddSkillTry(cid, 6, bonus * 100000)
		else	 
			doPlayerAddSkillTry(cid, 6, bonus * 120000)
		end 
	end
...
if getPlayerStorageValue(cid, 154585) >= 1 then
   doPlayerSendCancel(cid, "Voce ja esta pescando!")
   return true
end
...
if not isInArray({520, 521}, getCreatureOutfit(cid).lookType) then
	if getPlayerSex(cid) == PLAYERSEX_MALE then
		doSetCreatureOutfit(cid, {lookType = 520}, -1)
	else
		doSetCreatureOutfit(cid, {lookType = 521}, -1)
	end
   --return doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "You need fisher outfit for fishing.")
end
doCreatureSetLookDir(cid, getDirectionTo(getThingPos(cid), toPos))
local delay = fishing[getPlayerStorageValue(cid, sto_iscas)].segs
addEvent(doCreatureSetOutfit, delay * 1000, cid, myoutfit, -1)
