...
local pokemons = {
  ...
  {name = "Rhyhorn",  optionalLevel =  80, sex = SEX_MALE, nick = "", ball = "normal"},
...
function onCreatureSay(cid, type, msg)
  ...
  if msgcontains(msg, 'hi') and focus == 0 and getDistanceToCreature(cid) <= 4 then
    ...
    talk_start = os.clock()
    conv = 1
    selfSay("Ola "..getCreatureName(cid)..", , meu nome é Brock e eu sou o lider de ginasio de Pewter, como posso ajudar?")
    return true
  end
  ...
  if isDuelMsg(msg) and conv == 1 and focus == cid then
    ...
    if not hasPokemon(cid) then
      selfSay("Para batalhar contra um lider, voce precisa de um pokemon.")
      return true
    end
    selfSay("Voce esta me desafiando para uma batalha. Sera uma batalha com limite de " .. #pokemons ..
                    " pokemons. Vamos comecar?")
    conv = 2
    return true
  end
  ...
  if isNegMsg(msg) and conv == 2 and focus == cid then
    focus = 0
    selfSay("E melhor voce se recusar a lutar contra mim!")
    return true
  end
...
function onThink()
  ...
  if fighting then
    ...
    if not afk_warning and afk_time > afk_limit_time / 2 then
      selfSay("Cade o seu pokemon? Vamos lutar!")
      afk_warning = true
    end
    ...
    if not hasPokemon(challenger) or challenger_turn >= 7 or challenger_turn > #pokemons then
      gymClearFlags(getThis(), focus)
      selfSay("Voce perdeu, talvez alguma outra hora voce consiga.")
      setPlayerStorageValue(focus, 990, -1)
      focus = 0
      return true
    end
