...
function onCreatureSay(cid, type, msg)
  ...
  if msgcontains(msg, 'hi') and focus == 0 and getDistanceToCreature(cid) <= 4 then
    focus = cid; talk_start = os.clock(); conv = 1
    selfSay("Ola "..getCreatureName(cid)..", eu sou Sabrina, Lider do Ginasio de Saffron. Como posso ajudar?")
    return true
  end
  ...
  if isDuelMsg(msg) and conv == 1 and focus == cid then
    ...
    selfSay("Você esta me desafiando para uma batalha. Sera uma batalha com limite de " .. #pokemons ..
  ...
  if msgcontains(msg, 'bye') and focus == cid then
    selfSay('Tchau e boa sorte, treinador(a).')
...
function onThink()
  ...
  if fighting then
    ...
    if afk_time > afk_limit_time then
      ...
      setPlayerStorageValue(focus, 990, -1)
      focus = 0
      selfSay("Esperei demais, volte quando estiver pronto.")
      return true
    end
    ...
    if not hasPokemon(challenger) or challenger_turn >= 7 or challenger_turn > #pokemons then
      gymClearFlags(getThis(), focus)
      selfSay("Voce perdeu nosso duelo. Tente novamente mais tarde.")
      setPlayerStorageValue(focus, 990, -1)
      focus = 0
      return true
    end
