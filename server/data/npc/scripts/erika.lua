...
function onCreatureSay(cid, type, msg)
    ...
    if msgcontains(msg, 'hi') and focus == 0 and getDistanceToCreature(cid) <= 4 then
        ...
        talk_start = os.clock()
        conv = 1
        selfSay("Ola " .. getCreatureName(cid) .. ", eu sou a Erika, Lider do Ginasio de Celadon. Como posso ajudar?")
        return true
    end
...
function onThink()
    if focus == 0 then
        ...
        if #getCreatureSummons(getThis()) >= 1 then
            ...
            doCreatureAddHealth(getCreatureSummons(getThis())[1],
                -getCreatureMaxHealth(getCreatureSummons(getThis())[1]))
        end
        return true
    end
