...
if(NpcHandler == nil) then
		...
		if(callback == nil or callback()) then
			...
			if(self:processModuleCallback(CALLBACK_ONTHINK)) then
				if(NPCHANDLER_CONVBEHAVIOR ~= CONVERSATION_DEFAULT) then
					for pos, focus in pairs(self.focuses) do
						if(focus ~= nil) then
							if isCreature(focus) and (not self:isInRange(focus)) then
								self:onWalkAway(focus)
							elseif isCreature(focus) and ((os.time() - self.talkStart[focus]) > self.idleTime) then
								self:unGreet(focus)
							else
								self:updateFocus()
							end
				...
				elseif(self.focuses ~= 0) then
					if isCreature(self.focuses) and (not self:isInRange(self.focuses)) then
						self:onWalkAway(self.focuses)
					elseif isCreature(self.focuses) and (os.time()-self.talkStart > self.idleTime) then
	...
	function NpcHandler:onGreet(cid)
		if not isCreature(self.focuses) or self:isInRange(self.focuses) then
