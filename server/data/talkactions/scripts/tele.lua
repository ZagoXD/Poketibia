local poke = {"Shiny Mr. Mime", 'Mew', 'Mewtwo', 'Abra', 'Kadabra', 'Alakazam', 'Drowzee', 'Hypno', 'Mr. Mime',
              'Porygon', 'Shiny Abra', 'Shiny Alakazam', 'Shiny Hypno', 'Porygon2'} -- alterado v1.9
...
function onSay(cid, words, param)
    ...
    if type(eliteRunIsActive) == "function" and eliteRunIsActive(cid) then
        doPlayerSendCancel(cid, "You can't use Teleport during an Elite run!")
        return true
    end

    local RAID_STOR_ACTIVE = (RAID_STOR and RAID_STOR.ACTIVE) or 91000
    if getPlayerStorageValue(cid, RAID_STOR_ACTIVE) == 1 then
        doPlayerSendCancel(cid, "You can't use Teleport during raids!")
        return true
    end
    ...
    if getPlayerStorageValue(cid, IS_IN_TOURNAMENT) == 1 then
        doPlayerSendCancel(cid, "You can't do that while in the Tournament!")
        return true
    end
