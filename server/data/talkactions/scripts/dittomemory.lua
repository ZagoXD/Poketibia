            ...
            if slot and slot <= maxSlots and slot > 0 then
                if getDittoSlots(item)[slot] then
                    ...
                    doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE,
                        "[Ditto Memory] Slot " .. slot .. " deleted.")
            ...
            if #summons == 0 then
                ...
                if param:find("save") then
                    ...
                    if transformedPoke and type(transformedPoke) == "string" then
                        ...
                        if slot and slot <= maxSlots and slot > 0 then
                            if not getDittoSlots(item)[slot] and not hasDittoSavedPokemon(item, transformedPoke) then
                                ...
                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE,
                                    "[Ditto Memory] You saved at the slot " .. slot .. ": " .. transformedPoke .. ".")
                                sendDittoMemorySync(cid)
                            else
                                doPlayerSendCancel(cid,
                                    "This slot isn't empty or your Ditto already has this pokemon saved in his memory.")
                            end
                        else
                            doPlayerSendCancel(cid, "Invalid slot.")
                        end
                    ...
                    elseif getPlayerStorageValue(summons[1], 1010) == transformPoke then
                        ...
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "[Ditto Memory] Using the slot " ..
                            slot .. ", your Ditto transformed into a " .. transformPoke .. ".")

                        setPlayerStorageValue(summons[1], 1010, transformPoke)
                        doSetCreatureOutfit(summons[1], {
                            lookType = getPokemonXMLOutfit(transformPoke)
                        }, -1)
                        ...
                        doItemSetAttribute(item, "transLeft", -1)
                        doItemSetAttribute(item, "transBegin", os.clock())
                        for i = 1, 12 do
                            doItemEraseAttribute(item, "move" .. i)
                            doItemEraseAttribute(item, "cm_move" .. i)
                        end
                        doItemEraseAttribute(item, "active_moves")
                        ...
                        if dittoCopiesStatusToo then
                            setPlayerStorageValue(summons[1], 1001,
                                dittoBonus * pokes[transformPoke].offense * getPlayerLevel(cid))
                            ...
                            setPlayerStorageValue(summons[1], 1004,
                                dittoBonus * pokes[transformPoke].vitality * getPlayerLevel(cid))
                            setPlayerStorageValue(summons[1], 1005,
                                dittoBonus * pokes[transformPoke].specialattack * getPlayerLevel(cid))
