...
local STORAGE_BADGE_BASE = 9100 -- 9101..9108
...
function onExtendedOpcode(cid, opcode, buffer)
	...
	elseif opcode == opcodes.OPCODE_DITTO_MEMORY then
		 ...
		 local pokeName = getItemAttribute(item.uid, "poke")
			...
			if isInArray({"saveMemory1", "saveMemory2", "saveMemory3"}, buffer) then
			   ...
			   if isPokeInSlots(getItemAttribute(item.uid, "memoryDitto"), doCorrectString(copyName)) then doSendMsg(cid, "Esta copia j� est� salva em um slot.") return true end
	...
	elseif opcode == opcodes.OPCODE_TV_CAM then -- TVCam
			...
			elseif action == "watch" then
				...
				local playerToWatch = getCreatureByName(buffer:explode("/")[2])
				 ...
				 doPlayerSendCancel(cid, "Retorne seu pokemon para poder ver TV.") 
				 return true 
				 end	
				   if isCreature(playerToWatch) then
					 ...
					 doSendMsg(cid, "Este player n�o est� mais gravando.")
				   end
	...
	elseif opcode == opcodes.OPCODE_PLAYER_SHOW_TRADE_HELD then
        ...
        local posMachine = {{x = 137, y = 129, z = 3}, {x = 138, y = 129, z = 3}}
		...
		if op == 1 then
		   if doPlayerRemoveItem(cid, 13933, 20) then
		      local tier = math.random(1, 2)
		      doPlayerAddRandomHeld(cid, tier)
	           else
		      doSendMsg(cid, "Voc� precisa de 20 Devoted Tokens.")
		   end
		   return true 
		elseif op == 2 then
		   if doPlayerRemoveItem(cid, 13932, 70) then
		      local tier = math.random(1, 3)
		      doPlayerAddRandomHeld(cid, tier)
	           else
		      doSendMsg(cid, "Voc� precisa de 70 Mighty Tokens.")
		   end
		   return true
		elseif op == 3 then
		   if doPlayerRemoveItem(cid, 13932, 150) then
		      local tier = math.random(2, 4)
		      doPlayerAddRandomHeld(cid, tier)
	           else
		      doSendMsg(cid, "Voc� precisa de 150 Mighty Tokens.")
		   end
		   return true
		elseif op == 4 then
		   if doPlayerRemoveItem(cid, 13932, 300) then
		      local tier = math.random(2, 6)
		      doPlayerAddRandomHeld(cid, tier)
	           else
		      doSendMsg(cid, "Voc� precisa de 300 Mighty Tokens.")
		   end
		   return true 
		elseif op == 5 then
		   if doPlayerRemoveItem(cid, 13931, 30) then
		      local tier = math.random(1, 3)
		      doPlayerAddRandomHeld(cid, tier)
	           else
		      doSendMsg(cid, "Voc� precisa de 30 Hornored Tokens.")
		   end
		   return true 
		elseif op == 6 then
		   if doPlayerRemoveItem(cid, 13931, 75) then
		      local tier = math.random(2, 4)
		      doPlayerAddRandomHeld(cid, tier)
	           else
		      doSendMsg(cid, "Voc� precisa de 75 Hornored Tokens.")
		   end
		   return true
		elseif op == 7 then
		   if doPlayerRemoveItem(cid, 13931, 150) then
		      local tier = math.random(3, 6)
		      doPlayerAddRandomHeld(cid, tier)
	           else
		      doSendMsg(cid, "Voc� precisa de 150 Hornored Tokens.") -- doSendMsg
		   end
		   return true 
		end
	end
