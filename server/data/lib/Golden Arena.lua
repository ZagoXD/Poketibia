...
if getGlobalStorageValue(22550) >= 1 and getGlobalStorageValue(22548) <= 0 then
   if start then
      doBroadcastMessage("A Golden Arena começou!")    
   end
   ...
   setGlobalStorageValue(22547, wave+1) 
    ...
    if start then                                                    
       doBroadcastMessage("A Arena Dourada nao sera iniciada, pois nenhum jogador esta registrado!")
       return true
    end
end
end
...
for i = 1, #s do
    ...
    if isCreature(sid) then
       doTeleportThing(sid, getClosestFreeTile(sid, posGolden))  --pos pra onde os players vao ser teleportados quando a golden come�ar!
...
if not onStart then
   doBroadcastMessage("A Golden Arena terminou!")
end
end
------------------------------------------------------------------------------------------------------------------
...
local str = {}
table.insert(str, "Rank da Golden Arena:\n\n")
