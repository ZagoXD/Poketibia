...
function sendPokeHPMsg(cid, summon)
  if not isCreature(cid) then return end

  local hp, maxhp = 0, 0
  if summon and isCreature(summon) then
    hp, maxhp = getCreatureHealth(summon), getCreatureMaxHealth(summon)
  else
    local s = getCreatureSummons(cid)[1]
    if s and isCreature(s) then
      hp, maxhp = getCreatureHealth(s), getCreatureMaxHealth(s)
    end
  end
  if maxhp > 0 then
    local ratio = hp / maxhp
    local slotBall = getPlayerSlotItem(cid, CONST_SLOT_FEET)
    if slotBall.uid > 0 then
      doItemSetAttribute(slotBall.uid, "hp", ratio)
    end
  end

  doPlayerSendCancel(cid, POKE_HP_PREFIX .. hp .. "," .. maxhp)
end
