...
function doSendSleepEffect(cid)
    if not isCreature(cid) or not isSleeping(cid) then
        return true
    end
...
function doCondition2(ret)
    ...
    function doMiss2(cid, cd, eff, check, spell)
        ...
        if getPlayerStorageValue(cid, 21100) >= 1 and getPlayerStorageValue(cid, stg) <= -1 then
            return true
        end -- alterado v1.6  reflect
        if not canDoMiss(cid, spell) then
            return true
        end
    ...
    function doConfusion2(cid, cd, check)
        ...
        if isCreature(isTarget) and not isSleeping(cid) and not isParalyze(cid) and getPlayerStorageValue(cid, 654878) <=
            0 then -- alterado v1.6
    ...
    function doPoison2(cid, cd, check, damage)
        ...
        if isSummon(cid) or ehMonstro(cid) and pokes[getCreatureName(cid)] then -- alterado v1.6
            local type, type2 = getCurrentTypes(cid)
            ...
            if not isPlayer(cid) then
                ...
                if getCreatureName(cid) == "Ditto" and isSummon(cid) and tostring(dittoStg) and dittoStg ~= "Ditto" then
                    doSetCreatureOutfit(cid, {
                        lookType = getItemAttribute(getPlayerSlotItem(getCreatureMaster(cid), 8).uid, "transOutfit")
                    }, -1)
                else
                    doRemoveCondition(cid, CONDITION_OUTFIT)
                end
                if getPlayerStorageValue(cid, 625877) ~= -1 then
                    doSetCreatureOutfit(cid, {
                        lookType = getPlayerStorageValue(cid, 625877)
                    }, -1) -- alterado v1.6
                end
            end
            if isPlayer(cid) then
            ...
            elseif not isPlayer(cid) then
                ...
                if getCreatureName(cid) == "Ditto" and isSummon(cid) and tostring(dittoStg) and dittoStg ~= "Ditto" then
                    ...
                    if InfoDitto ~= 0 and look.lookType ~= 0 then
                        doSetCreatureOutfit(cid, {
                            lookType = 0,
                            lookTypeEx = getMonsterInfo(tostring(dittoStg)).lookCorpse
                        }, -1)
                    end
                else
    ...
    function doBuff2(cid, cd, eff, check, buff, first, attr)
        ...
        if a <= -1 then -- alterado v1.6
            if isInArray({"Future Sight", "Camouflage", "Acid Armor", "Iron Defense", "Minimize", "Bug Fighter"}, buff) then
            ...
            if isInArray({"Strafe", "Agility", "War Dog", "Fighter Spirit", "Furious Legs", "Ultimate Champion",
                          "Bug Fighter"}, buff) then
...
if isNpcSummon(pid) then
  local t = getCreatureTarget(pid)
end
...
local atkMaster = isSummon(cid) and getCreatureMaster(cid) or cid
local defMaster = isSummon(pid) and getCreatureMaster(pid) or pid

  local atkIsGym = isNpcSummon(cid)

  local defPlayer = isSummon(pid) and getCreatureMaster(pid) or pid
  local defIsGymPlayer = isPlayer(defPlayer) and getPlayerStorageValue(defPlayer, 990) >= 1

  if atkIsGym and defIsGymPlayer then
      if isPlayer(pid) and #getCreatureSummons(pid) >= 1 then
          local s = getCreatureSummons(pid)[1]
          if isCreature(s) then pid = s end
      end

      local realAttacker = isCreature(atkMaster) and atkMaster or cid
      ...
      if canAtk then
          ...
          if spell == "Selfdestruct" then
              if getPlayerStorageValue(pid, 9658783) <= 0 then
                  ...
                  doTargetCombatHealth(realAttacker, pid, element, -math.abs(max), -math.abs(max), 255)
              end
          else
              doTargetCombatHealth(realAttacker, pid, element, -math.abs(min), -math.abs(max), 255)
          end
      end
...
local gymOk =
  (isNpcSummon(pid) and (getCreatureTarget(pid) == cid or getCreatureTarget(pid) == atkMaster)) or
  (isNpcSummon(cid) and (getCreatureTarget(cid) == pid or getCreatureTarget(cid) == defMaster))
-- if isNpcSummon(cid) or isNpcSummon(pid) then
--   print("[GYM DMG] spell="..tostring(spell)..
--         " atk="..cid.."("..getCreatureName(cid)..")"..
--         " atkT="..tostring(getCreatureTarget(cid))..
--         " def="..pid.."("..getCreatureName(pid)..")"..
--         " defT="..tostring(getCreatureTarget(pid))..
--         " gymOk="..tostring(gymOk))
-- end
if isSummon(cid) and (
    ehMonstro(pid) or
    gymOk or
    (isSummon(pid) and canAttackOther(cid, pid) == "Can") or
    (isPlayer(pid) and canAttackOther(cid, pid) == "Can" and #getCreatureSummons(pid) <= 0)
) and pid ~= cid then
         if canAtk then     --alterado v1.6
            ...
            if spell == "Selfdestruct" then
               if getPlayerStorageValue(pid, 9658783) <= 0 then
                  ...
                  local attacker = isSummon(cid) and getCreatureMaster(cid) or cid
                  doTargetCombatHealth(attacker, pid, element, -math.abs(max), -math.abs(max), 255)
               end
            else
...
function canDoMiss(cid, nameAtk) -- alterado v1.5
    ...
    local t1, t2 = getCurrentTypes(cid)

    if isInArray(atkTerra, nameAtk) then
        if (t1 == "flying") or (t2 == "flying") or isInArray(specialabilities["levitate"], getCreatureName(cid)) then
            return false
        end
    elseif isInArray(atkElectric, nameAtk) then
        if (t1 == "ground") or (t2 == "ground") then
            return false
        end
    end
    ...
    if isCreature(cid) and isCreature(pid) then
        if getPlayerStorageValue(cid, 374896) >= 1 then
            if getMasterTarget(cid) == pid then
                ...
                if isSummon(cid) then
                    doTargetCombatHealth(getCreatureMaster(cid), pid, PHYSICALDAMAGE, -math.abs(valor),
                        -math.abs(valor), 255)
                else
