...
function recheck(sid, skill, pos)
	...
	elseif skill == "fly" then
	    ...
	    --return true
	    --end
		if not flys[getPokemonName(getCreatureSummons(cid)[1])] then
			return false
		end
		...
		local flyheld = getHeldBonusValue(getPlayerSlotItem(cid, 8).uid, 'y', 'Wing')
		local speed = ((pokemon[2]) * flysprate) + flyheld --local speed = ((NPSpeed + pokemon[2]) * flysprate) + flyheld  --local speed = ((pokemon[2]) * flysprate) + flyheld
		...
		if summon and isCreature(summon) then
...
if getDistanceBetween(getThingPos(sid), getThingPos(getCreatureMaster(sid))) <= 1 then
