...
local function findMoveEntryOnMovestable(pokeName, move)
  local x = movestable[pokeName]
...
local function enrichFromMovestable(baseMoveTbl, pokeName, move)
  local info = shallowcopy(baseMoveTbl or {})
  local ent = findMoveEntryOnMovestable(pokeName, move)

  if ent then
    if info.target == nil and ent.target ~= nil then info.target = ent.target end
    if info.dist   == nil and ent.dist   ~= nil then info.dist   = ent.dist   end
    if info.cd     == nil and ent.cd     ~= nil then info.cd     = ent.cd     end
    if info.t      == nil and ent.t      ~= nil then info.t      = ent.t      end
    if info.f      == nil and ent.f      ~= nil then info.f      = ent.f      end
  end
...
function getTableMove(cid, move)               --alterado v1.6
  ...
  if getPlayerStorageValue(cid, 21103) ~= -1 then
   ...
   for j = 1, 15 do
   if z[j] and z[j].name == move then
      local mi = movesinfo[move]
      if mi then
         local r = shallowcopy(z[j])
         if r.cat    == nil and mi.cat    ~= nil then r.cat    = mi.cat    end
         if r.f      == nil and mi.f      ~= nil then r.f      = mi.f      end
         if r.t      == nil and mi.t      ~= nil then r.t      = mi.t      end
         if r.target == nil and mi.target ~= nil then r.target = mi.target end
         if r.dist   == nil and mi.dist   ~= nil then r.dist   = mi.dist   end
         if r.cd     == nil and mi.cd     ~= nil then r.cd     = mi.cd     end
         if r.target == nil then r.target = 1 end
         return r
      end
      return z[j]
   end
  ...
  local mi = movesinfo[move]
  if mi then
    return enrichFromMovestable(mi, name, move)
  end
...
local moveTbl = getTableMove(cid, spell) --alterado v1.6
...
elseif not isPlayer(cid) then
    if moveTbl ~= "" then
        ...
        local function pickBaseStat(cid, tbl)
            local cat = (tbl and tbl.cat) or "special"   -- fallback
            if cat == "physical" then
                return getOffense(cid)
            else
                return getSpecialAttack(cid)
            end
        end

        local baseStat = pickBaseStat(cid, moveTbl)
        min = baseStat * moveTbl.f * 0.1
...
--- FOCUS ----------------------------------            
if getPlayerStorageValue(cid, 253) >= 0 and moveTbl ~= "" and moveTbl.f ~= 0 then
	min = min * 2
	max = max * 2
...
elseif spell == "Demon Kicker" then
  ...
  local chanceTbl = passivesChances["Demon Kicker"]
  local chance = (chanceTbl and chanceTbl[nome]) or 0

  if (not hitmonlees[nome] and isCreature(target)) or (isCreature(target) and math.random(1, 100) <= chance) then
    ...
    local function doChangeHitmon(_cid)
      if not isCreature(_cid) then return true end
      setPlayerStorageValue(_cid, 32623, 0)

      local mi = getMonsterInfo(getCreatureName(_cid))
      if isSleeping(_cid) and mi and mi.lookCorpse ~= 0 then
        doSetCreatureOutfit(_cid, {lookType = 0, lookTypeEx = mi.lookCorpse}, -1)
      else
        doRemoveCondition(_cid, CONDITION_OUTFIT)
      end
    end

    setPlayerStorageValue(cid, 32623, 1)

    local look = hitmonlees[nome]
    if not look or look == 0 then
      local st = getPlayerStorageValue(cid, 21104)
      look = tonumber(st) or 0
    end
...
elseif spell == "Demon Puncher" then
  ...
  if (not hitmonchans[name] and isCreature(target)) or
     (isCreature(target) and math.random(1, 100) <= (passivesChances["Demon Puncher"] and passivesChances["Demon Puncher"][name] or 0)) then
    ...
    if ehMonstro(cid) or not hitmonchans[name] then
      ...
      local boots = getPlayerSlotItem(master, 8)
      if not boots or boots.uid <= 0 then
        hands = 0
      else
        local h = getItemAttribute(boots.uid, "hands")
        hands = tonumber(h) or 0
      end
    end

    if not hitmonchans[name] then
      local tname = getCreatureName(target)
      if not hitmonchans[tname] then
        return true
      end
      tabela = hitmonchans[tname][hands]
    else
      tabela = hitmonchans[name][hands]
    end

    if not tabela or not tabela.type or not tabela.eff then
      return true
    end
    -- =========================
