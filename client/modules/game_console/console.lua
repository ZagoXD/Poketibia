...
function init()
    ...
    local chatLocked = true

    local function lockChat()
        chatLocked = true
        if consoleTextEdit then
            if consoleTextEdit.disable then
                consoleTextEdit:disable()
            end
            if consoleTextEdit.setFocused then
                consoleTextEdit:setFocused(false)
            end
        end
    end
    ...
    local function unlockChat()
        chatLocked = false
        if consoleTextEdit then
            if consoleTextEdit.enable then
                consoleTextEdit:enable()
            end
            if consoleTextEdit.focus then
                consoleTextEdit:focus()
            end
        end
    end
    consoleTextEdit.onMousePress = function(self, mousePos, mouseButton)
        unlockChat()
        return false
    end
    local prevRootMousePress = root.onMousePress

    root.onMousePress = function(self, mousePos, mouseButton)
        lockChat()
        if type(prevRootMousePress) == 'function' then
            return prevRootMousePress(self, mousePos, mouseButton)
        end
    ...
    g_keyboard.bindKeyDown('Enter', function()
        ...
        if consoleTextEdit and not consoleTextEdit:isFocused() then
            unlockChat()
            return true
        end
...
function removeTabTv()
    for _, tab in ipairs(consoleTabBar:getChildren()) do
        if tab:getText() and string.find(tab:getText(), "%[TV%]") then
            removeTab(tab)
...
function removeTab(tab)
    ...
    if string.find(tab:getText(), "%[TV%]") then
...
function addText(text, speaktype, tabName, creatureName)
    if type(text) == 'string' and (text:find('^%[DMEM%]') or text:find('^%[DMEM_HIDE%]')) then
        return
    end
...
function onTalk(name, level, mode, message, channelId, creaturePos)
    ...
    if (mode == MessageModes.Say or mode == MessageModes.Whisper or mode == MessageModes.Yell or mode ==
        ...
        MessageModes.NpcFrom or mode == MessageModes.BarkLow or mode == MessageModes.BarkLoud) and creaturePos then
