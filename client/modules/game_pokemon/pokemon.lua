...
local function setPokeBar(hp, maxHp)
  if not pokeHealthBar then return end
  ...
  if maxHp <= 0 then
    ...
    pokeHealthBar:setValue(0, 0, 1)
    pokeHealthBar:setTooltip('')
  else
    ...
    pokeHealthBar:setTooltip(tr(pokeHealthTooltip, hp, maxHp))
  end
end
...
function init()
  ...
  pokemonButton = modules.client_topmenu.addRightGameToggleButton(
    'pokemonButton',
    tr('Pokemon') .. ' (Ctrl+P)',
    '/images/topbuttons/pokemon',
    toggle
  )
...
function onPokeHealthChange(mode, text)
  ...
  if not text:find(POKE_HP_PREFIX, 1, true) then return end

  local t = text:explode(',')
  local hp    = tonumber(t[2]) or 0
  local maxHp = tonumber(t[3]) or 0
  setPokeBar(hp, maxHp)
end
...
function onSetFightMode(self, selectedFightButton)
  ...
  if buttonId == 'fightOffensiveBox' then
    ...
    g_game.talk("@changedmg ".. 1)
  elseif buttonId == 'fightBalancedBox' then
    fightMode = FightBalanced
    g_game.talk("@changedmg ".. 2)
  else
    fightMode = FightDefensive
    g_game.talk("@changedmg ".. 3)
  end
